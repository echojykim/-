<template>
  <div>
    ...
    <table border="1">
      ...
      <tr v-if="!basicBoard || (Array.isArray(basicBoards) && basicBoards.length === 0)">
        <!-- v-if는 vue에서 제공하는 if조건문. 어레이 배열을 불러왔을때 길이가 0이면 아래 현재 등록된 게시물이 없습니다라고 뜨게 해주세요-->
        <td colspan="4">현재 등록된 게시물이 없습니다!</td>
      </tr>
      <tr v-else v-for="basicBoard in basicBoards" :key="basicBoard.boardNo">
        <!-- for문이 v-for문 -->
        <td align="center">
          {{ basicBoard.boardNo }}
        </td>
        <td align="left">
          <router-link
            :to="{
              name: 'BoardReadPage',
              params: { boardNo: basicBoard.boardNo.toString() },
            }"
          >
...
export default {
  ...
  beforeUpdate() {
    console.log("check: " + this.basicBoards);
  },
};
</script>
